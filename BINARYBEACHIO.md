...
| `Dockerfile` | Two from-source build fixes in the go-builder stage. **(1)** Pin `bingo` to `v0.9.0` instead of `@latest` (line 68). Upstream's `go install github.com/bwplotka/bingo@latest` worked when Grafana 11.2.0 shipped (Sept 2024) but bingo subsequently bumped to v0.10.0 which requires Go ≥ 1.24.9; Grafana's Dockerfile pins `golang:1.22.4-alpine`, so the build fails today. v0.9.0 is the last release before the Go floor bump. **(2)** Add `bash` to the `apk add` install list (line ~46). Grafana's `make build-go` target invokes `scripts/go-workspace/update-workspace.sh` which has a `#!/usr/bin/env bash` shebang. Alpine ships only busybox sh, so without this the build fails with `make: bash: No such file or directory`. Upstream never noticed because their CI uses the tgz-builder path (consuming prebuilt tarballs from Drone), so this go-builder path is only exercised by from-source builders like ours. **Build-system patches only**, not runtime patches — the running Grafana binary is unaffected. Becomes obsolete when upstream bumps GO_IMAGE past 1.24.9 + pins bingo themselves AND upstream either adds bash to the alpine deps or removes the bash dependency from `update-workspace.sh`. | ~3 | None — additive `apk` package + single-line version pin; refresh-from-upstream will re-introduce `@latest` and remove bash, needing both re-applied. |
